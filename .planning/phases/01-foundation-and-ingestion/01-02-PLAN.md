...
must_haves:
  truths:
    ...
    - "Clicking Dismiss updates the bug status to dismissed, changes the embed to grey color, and disables only the Dismiss button while the other three buttons remain in their existing disabled state"
...
- If action == "dismiss": handle dismiss flow
  ...
  6. Rebuild the view with `build_bug_view(self.bug_id, dismissed=True)` -- see build_bug_view below for button state logic
...
- Button states per CONTEXT.md decision "Buttons stay active after use -- users can re-trigger actions if needed":
  - **When dismissed=False (initial state):** Dismiss button is enabled (active, danger style). Analyze, Create Issue, and Draft Fix buttons are disabled (Phase 2-3 features per CONTEXT.md).
  - **When dismissed=True:** Dismiss button becomes disabled (it has been used, cannot re-dismiss). Analyze, Create Issue, and Draft Fix remain disabled (same as before -- they were already disabled). All 4 buttons remain visible in the view; no buttons are removed or hidden.
  - This honors the CONTEXT.md decision: buttons stay visible and present after use. The Dismiss button transitions from active to disabled upon use. The other three buttons were already disabled and remain so.
- Return view
...
3. Click Dismiss with a user who has the Developer role -- embed should turn grey, Dismiss button becomes disabled, other 3 buttons remain disabled as before, confirmation in thread
...
6. Test role gating:
   ...
   - Click Dismiss with the Developer role -- embed should turn grey, Dismiss button becomes disabled, other 3 buttons stay disabled as they were, confirmation in thread
...
8. Verify items to confirm:
   ...
   - All 4 buttons remain visible after Dismiss (none removed or hidden)
  </verify>
  <done>
User has confirmed: webhook ingestion works, embeds display correctly with summary-only content, threads contain full details, buttons are functional with role gating, dismiss flow works (Dismiss button disabled, others unchanged), and buttons persist across bot restarts. Phase 1 is complete.
  </done>
</task>
...
6. Dismiss changes embed to grey and disables only the Dismiss button; all 4 buttons remain visible
...
- Dismiss flow: button click -> status update -> embed refresh (grey) -> Dismiss button disabled -> thread notification
